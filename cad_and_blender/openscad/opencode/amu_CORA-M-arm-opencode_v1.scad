    ...
    difference() {
        union() {
            ...
            for (x = [-MMS_LENGTH*0.38 : 3.0 : MMS_LENGTH*0.38])
                for (a = [0:60:300]) {
                    if ((abs(x - ROOF_HATCH_X) > 1.6 || !(a == 90 || a == 60)) && !(abs(x - 1.2) < 2.45 && a == 90)) {
        ...
        translate([1.2, 0, MMS_RADIUS - 0.21])
